...
if __name__ == "__main__":
    ...
    while True:
        ...
        if decrypted[4] != 0x0d or (sum(decrypted[:3]) & 0xff) != decrypted[3]:
            print(
            hd(data), " => ", hd(decrypted), "Checksum error")
        else:
            op = decrypted[0]
            ...
            print(
            ", ".join("%s%02X: %04X %5i" % ([" ", "*"][op == k], k, v, v) for (k, v) in sorted(values.items())), "  ",)
            ...
            if 0x42 in values:
                print(
                "T: %2.2f" % (values[0x42] / 16.0 - 273.15),)
            if 0x44 in values:
                print(
                "RH: %2.2f" % (values[0x44] / 100.0),)
            print()